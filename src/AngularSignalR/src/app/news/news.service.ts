...
export class NewsService {
    ...
    private init() {
        ...
        this._hubConnection.on('Send', (newsItem: NewsItem) => {
          this.store.dispatch(newsAction.recieveNewsItemAction({payload: newsItem}));
        });

        this._hubConnection.on('JoinGroup', (data: string) => {
          console.log('recieved data from the hub');
          console.log(data);
          this.store.dispatch(newsAction.recieveGroupJoinedAction({payload:data}));
        });
        ...
        this._hubConnection.on('History', (newsItems: NewsItem[]) => {
          ...
          this.store.dispatch(newsAction.recieveNewsGroupHistoryAction({payload:newsItems}));
        });
    }
